...
Lparms_M = np.zeros(12, dtype='int')  # array of dimensions etc.
Lparms_M[0] = Npix
Lparms_M[1] = NSteps
Lparms_M[2] = Nf

Rparms = np.zeros(5, dtype='float')  # array of global floating-point parameters - for a single LOS
...
Rparms_M = np.zeros((5, Npix), dtype='float', order='F')  # 2D array of global floating-point parameters - for multiple LOSs
...
Parms_M = np.zeros((24, NSteps, Npix), dtype='float', order='F')  # 3D array of input parameters - for multiple voxels and LOSs
for pix in range(Npix):
...
Parms_M = Parms_M.flatten('F')

RL_M = np.zeros((7, Nf, Npix), dtype='float', order='F')  # input/output array
RL_M = RL_M.flatten('F')

dummy = np.array(0, dtype='float')

# calculating the emission for analytical distribution (array -> off),
# the unused parameters can be set to any value
if __name__ == "__main__":
    res = interface.getMWSlice(Lparms_M, Rparms_M, Parms_M, dummy, dummy, dummy, RL_M)

# plotting the results

RL_M = RL_M.reshape((7, Nf, Npix), order = 'F')
