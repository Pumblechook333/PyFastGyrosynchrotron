...
E_arr = np.logspace(np.log10(Emin), np.log10(Emax), N_E, dtype=float)  # energy grid (logarithmically spaced)
mu_arr = np.linspace(-1.0, 1.0, N_mu, dtype=float)  # pitch-angle grid

f0 = np.zeros((N_E, N_mu), dtype=float)  # 2D distribution function array - for a single voxel
...
f_arr = np.zeros((N_E, N_mu, NSteps), dtype=float, order='F')  # 3D distribution function array - for multiple voxels
...
res = interface.getMW(Lparms, Rparms, flatParms, E_arr, mu_arr, f_arr, RL)
