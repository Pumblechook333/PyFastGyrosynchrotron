...
Rparms_M = np.zeros((5, Npix), dtype='double',
                    order='F')  # 2D array of global floating-point parameters - for multiple LOSs
for pix in range(Npix):
    Rparms_M[:, pix] = Rparms  # the parameters are the same for all LOSs
Rparms_M = Rparms_M.flatten()
...
Parms_M = np.zeros((24, NSteps, Npix), dtype='double',
                   order='F')  # 3D array of input parameters - for multiple voxels and LOSs
for pix in range(Npix):
    Parms_M[:, :, pix] = Parms  # parameters are the same for all LOSs

Parms_M = Parms_M.flatten()
...
for k in range(Npix):
    ...
    mu_c = np.cos(
        (90.0 - 25.0 * k) * np.pi / 180)  # loss-cone angle decreases from 90 to 15 degrees, depending on the LOS
    ...
    for l in range(NSteps):
        f_arr_M[:, :, l,
        k] = f0  # electron distribution function is the same for all voxels within a LOS, but varies from LOS to LOS
f_arr_M = f_arr_M.flatten()

RL_M = np.zeros((7, Nf, Npix), dtype='double', order='F')  # input/output array
RL_M = RL_M.flatten()

# calculating the emission for array distribution (array -> on)
res = interface.getMWSlice(Lparms_M, Rparms_M, Parms_M, E_arr, mu_arr, f_arr_M, RL_M)

# plotting the results
RL_M = RL_M.reshape((7, Nf, Npix))
